...
class classify(object):
    ...
    def category(self):
        ...
        drilling_wells = []
        workover_wells = []
        #print('drilling rigs',drilling_rigs,workover_rigs)
        ...
        #print('workover',drilling_wells,workover_wells)
        ...
        #print(mid_drilling_rigs)
        #print(high_drilling_rigs)
        ...
        #print(mid_drilling_wells)
        #print(high_drilling_wells)
        drilling_rig_assignments = {}
        workover_rig_assignments = {}
        while(len(low_drilling_wells)!=0):
            for rig_name in low_drilling_rigs:
                for well_name in low_drilling_wells:
                    if well_name["depth_capacity"] <= rig_name["depth_capacity"]:
                        drilling_rig_assignments[well_name["id"]] = rig_name["name"]
                        low_drilling_wells.remove(well_name)
                        break
                    # compare with last
                    if rig_name == low_drilling_rigs[-1]:
                        low_drilling_wells.remove(well_name)
        while(len(mid_drilling_wells)!=0):
            for rig_name in mid_drilling_rigs:
                for well_name in mid_drilling_wells:
                    if well_name["depth_capacity"] <= rig_name["depth_capacity"]:
                        drilling_rig_assignments[well_name["id"]] = rig_name["name"]
                        mid_drilling_wells.remove(well_name)
                        break
                    # compare with last rig_name
                    if rig_name == mid_drilling_rigs[-1]:
                        mid_drilling_wells.remove(well_name)
        while(len(high_drilling_wells)!=0):
            for rig_name in high_drilling_rigs:
                for well_name in high_drilling_wells:
                    if well_name["depth_capacity"] <= rig_name["depth_capacity"]:
                        drilling_rig_assignments[well_name["id"]] = rig_name["name"]
                        high_drilling_wells.remove(well_name)
                        break
                    # compare with last rig_name
                    if rig_name == high_drilling_rigs[-1]:
                        high_drilling_wells.remove(well_name)
                
        while(len(workover_wells)!=0):        
            for rig_name in workover_rigs:
                for well_name in workover_wells:
                    if well_name["depth_capacity"] <= rig_name["depth_capacity"]:
                        workover_rig_assignments[well_name["id"]] = rig_name["name"]
                        workover_wells.remove(well_name)
                        break
                    # compare with last rig_name
                    if rig_name == workover_rigs[-1]:
                        workover_wells.remove(well_name)
        ...
        for well_id, rig_name in workover_rig_assignments.items():
            rig_wise_allotted_well_ids[rig_name].append(well_id)
        print("Rig wise allotted wells -->", rig_wise_allotted_well_ids)
        result = {k:v for k,v in rig_wise_allotted_well_ids.items() if v}
        #print(result)
        return result
...
##          date =s.strftime("%d-%m-%Y %H:%M:%S.%f")
    ...
    def solveNCW(self,r,sample,sample1,wellsDict,rigsDict,r_name,rigs_dict):
        w1_opt=[]
        d_opt=[]
        total_loss=[]
        sample3=[]
        ...
        #print("travel time sequence :", t_opt)
        self.schedule(r,wellsDict,rigsDict,t_opt,r_name)
    ...
    def well_assign(self):
        ...
        print('optimal wells sequence: ', r)
        #print(r_name)
        return self.solveNCW(r,rigs_location,wells_location,wellsDict,rigsDict,r_name,rigs_dict)        
if __name__ == "__main__":
    ...
    rig1 = Rig("0","Rig1","Drilling", 9200,2600,10,15000,{'rig_location':[52.316518116281095,24.511424873474734],'rig_maintance':4,'rig_init_status':'mastUp'})
    rig2 = Rig("1","Rig2","Drilling", 19260,4500,12,20000,{'rig_location':[52.32017234725687,24.513385352172136],'rig_maintance':4,'rig_init_status':'mastDown'})
    rig3 = Rig("2","Rig3","Workover", 1400,1150,5,8000,{'rig_location':[52.316518116281095,24.511424873474734],'rig_maintance':4,'rig_init_status':'mastUp'})
    rig4 = Rig("3","Rig4","Workover", 1300,1100,6,9000,{'rig_location':[52.31725062702516,24.504785735789866],'rig_maintance':4,'rig_init_status':'mastDown'})
##    rig5 = Rig("5","Rig5","Workover", 1200,1160,5,7000,{'rig_location':'(145 , 226)','rig_maintance':4,'rig_init_status':'mastUp'})
##    rig6 = Rig("6","Rig6","Drilling", 5200,1400,6,9000,{'rig_location':'(313 , 758)','rig_maintance':4,'rig_init_status':'mastDown'})
##    rig7 = Rig("7","Rig7","Drilling", 18000,5000,5,7000,{'rig_location':'(145 , 226)','rig_maintance':4,'rig_init_status':'mastUp'})
...
##    well1 = Well('0', 'well1', 'Drilling', 3000, {'startTime': '22-02-2023', 'endTime': '6-03-2023', 'workoverTime': 2, 'priority': 4, 'well_location': '(641 , 125)', 'lossFactor': 0.26})
##    well2 = Well('1', 'well2', 'Drilling', 9000, {'startTime': '2-02-2023', 'endTime': '8-03-2023', 'workoverTime': 3, 'priority': 1, 'well_location': '(611 , 377)', 'lossFactor': 0.14})
##    well3 = Well('2', 'well3', 'Drilling', 5000, {'startTime': '27-02-2023', 'endTime': '21-03-2023', 'workoverTime': 2, 'priority': 5, 'well_location': '(383 , 505)', 'lossFactor': 0.38})
##    well4= Well('3', 'well4', 'Drilling', 7642, {'startTime': '14-02-2023', 'endTime': '23-03-2023', 'workoverTime': 3, 'priority': 5, 'well_location': '(302 , 133)', 'lossFactor': 0.48})
##    well5 = Well('4', 'well5', 'Drilling', 10219, {'startTime': '28-02-2023', 'endTime': '1-03-2023', 'workoverTime': 3, 'priority': 3, 'well_location': '(517 , 370)', 'lossFactor': 0.11})
##    well6 = Well('5', 'well6', 'Workover', 1200, {'startTime': '19-02-2023', 'endTime': '21-03-2023', 'workoverTime': 2, 'priority': 1, 'well_location': '(154 , 148)', 'lossFactor': 0.41})
##    well7 = Well('6', 'well7', 'Drilling', 14637, {'startTime': '9-02-2023', 'endTime': '4-03-2023', 'workoverTime': 5, 'priority': 5, 'well_location': '(559 , 821)', 'lossFactor': 0.27})
##    well8 = Well('7', 'well8', 'Workover', 1300, {'startTime': '11-02-2023', 'endTime': '19-03-2023', 'workoverTime': 4, 'priority': 5, 'well_location': '(997 , 141)', 'lossFactor': 0.23})
##    well9 = Well('8', 'well9', 'Drilling', 8000, {'startTime': '27-02-2023', 'endTime': '7-03-2023', 'workoverTime': 2, 'priority': 1, 'well_location': '(931 , 241)', 'lossFactor': 0.17})
##    well10 = Well('9', 'well10', 'Workover', 800, {'startTime': '2-02-2023', 'endTime': '28-03-2023', 'workoverTime': 4, 'priority': 5, 'well_location': '(726 , 185)', 'lossFactor': 0.1})
##            ('0', 'well1', 'Drilling', 1453, {'startTime': '18-02-2023', 'endTime': '22-03-2023', 'workoverTime': '3', 'priority': '5', 'well_location': '(399 , 982)', 'lossFactor': '0.44'})
    well1 = Well('0', 'well1', 'Drilling', 3075, {'startTime' : '7-02-2023',' endTime ': '16-03-2023', 'workoverTime' : 2, 'priority' : 5, 'well_location' : [52.31905567855765,24.504293593816882], 'lossFactor' : 0.31})
    well2 = Well("1","well2","Drilling", 2200,{'startTime' : '23-02-2023', 'endTime' : '10-03-2023','workoverTime' : 3,'priority' : 1 ,'well_location':[52.31885743004541,24.50547982157096],'lossFactor':0.51})
    well3 = Well("2","well3","Workover", 1000,{'startTime' : '25-02-2023', 'endTime' : '10-03-2023', 'workoverTime' : 2,'priority' : 4,'well_location':[52.318000902355266,24.506923440798918],'lossFactor':0.93})
    well4 = Well("3","well4","Workover", 1150,{'startTime' : '23-02-2023', 'endTime' : '10-03-2023', 'workoverTime' : 2,'priority' : 3,'well_location':[52.31704737951449,24.50588349616796],'lossFactor':0.77})
    well5 = Well("4","well5","Workover", 1400,{'startTime' : '24-02-2023', 'endTime' : '10-03-2023', 'workoverTime' : 4,'priority' : 5,'well_location':[52.31491825718931,24.508015569516],'lossFactor':0.48})
    well6 = Well("5","well6","Drilling", 2050,{'startTime' : '23-02-2023', 'endTime' : '10-03-2023','workoverTime' : 2,'priority' : 8,'well_location':[ 52.31643698447878,24.50493771512199],'lossFactor':0.24})
    well7 = Well("6","well7","Drilling", 2250,{'startTime' : '23-02-2023', 'endTime' : '10-03-2023','workoverTime' : 3,'priority' : 6 ,'well_location':[52.32260012449841,24.508511293513372],'lossFactor':0.51})
    well8 = Well("7","well8","Workover", 1100,{'startTime' : '23-02-2023', 'endTime' : '10-03-2023', 'workoverTime' : 2,'priority' : 9,'well_location':[52.33905567855765,24.504293593816882],'lossFactor':0.93})
    well9 = Well("8","well9","Workover", 1175,{'startTime' : '23-02-2023', 'endTime' : '10-03-2023', 'workoverTime' : 2,'priority' : 7,'well_location':[52.348000902355266,24.506923440798918],'lossFactor':0.77})
    ...
    well11 = Well("10","well11","Drilling", 2250,{'startTime' : '23-02-2023', 'endTime' : '10-03-2023','workoverTime' : 3,'priority' : 6 ,'well_location':[52.33491825718931,24.508015569516],'lossFactor':0.51})
    well12 = Well("11","well12","Workover", 1100,{'startTime' : '23-02-2023', 'endTime' : '10-03-2023', 'workoverTime' : 2,'priority' : 9,'well_location':[52.34643698447878,24.50493771512199],'lossFactor':0.93})
    well13 = Well("12","well3","Workover", 1175,{'startTime' : '23-02-2023', 'endTime' : '10-03-2023', 'workoverTime' : 2,'priority' : 7,'well_location':[52.33260012449841,24.508511293513372],'lossFactor':0.77})
    well14 = Well("13","well14","Workover",1450,{'startTime' : '23-02-2023', 'endTime' : '10-03-2023','workoverTime' : 4,'priority' : 10,'well_location':[52.34260012449841,24.508511293513372],'lossFactor':0.48})
    scheduler = Scheduler(
        [rig1,rig2,rig3,rig4],
